...
export class HttpController extends Http {
  ...
  request(url: string | Request, options?: RequestOptionsArgs): Observable<Response> {
    if (typeof url !== "string") {
      (<Request> url).url = this.getServerUrl((<Request> url).url);
    } else {
      url = this.getServerUrl(url);
    }
    this.setAuthorizationHeader(url, options);
    return new Observable((observer: PartialObserver<any>) => {
      super.request(url, options).subscribe((res) => observer.next(res),
  ...
  public authenticate(username: string, password: string): Observable < Request > {
    return new Observable((observer: PartialObserver<any>) => {
      ...
      super.request(this.getServerUrl("/login"), options).map((res: Response) => res.json())
        .subscribe((res: any) => {
